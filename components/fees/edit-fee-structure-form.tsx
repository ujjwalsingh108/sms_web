...
export default function EditFeeStructureForm({
  ...
  const form = useForm<FeeStructureFormData>({
    ...
    defaultValues: {
      ...
      due_day: feeStructure.due_day?.toString() || "",
  ...
  const onSubmit = async (data: FeeStructureFormData) => {
    setIsLoading(true);
    try {
      const result = await updateFeeStructure(feeStructure.id, {
        ...data,
      ...
      if (result.success) {
        ...
        router.push(`/dashboard/fees/structures/${feeStructure.id}`);
      } else {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex gap-4">
            ...
            <Link
              href={`/dashboard/fees/structures/${feeStructure.id}`}
              className="flex-1"
            >
